...
export const scrapComponent = async (keyword: string, page: number) => {
    ...
    if (rest === 1) {
        ...
        const page_2 = (await makeList(keyword, start + 1)).slice(0, 20); //20개로 자른 리스트
        return [...page_1, ...page_2];
    } else if (rest === 2) {
        const start = 2 + 5 * Math.floor(page / 3);
        const page_1 = (await makeList(keyword, start)).slice(20); //10개짜리 리스트
        const page_2 = await makeList(keyword, start + 1); //30개짜리 리스트
        const page_3 = (await makeList(keyword, start + 2)).slice(0, 10); //0~10번째 인덱스까지 자른 리스트
        return [...page_1, ...page_2, ...page_3];
    } else if (rest === 0) {
        const start = -1 + 5 * Math.floor(page / 3);
        const page_1 = (await makeList(keyword, start)).slice(10); //20개
        const page_2 = await makeList(keyword, start + 1); //30개
        return [...page_1, ...page_2];
    }
};
